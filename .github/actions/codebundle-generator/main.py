...
class CodebundleGenerator:
    def __init__(self, issue_number: int, github_token: str, openai_api_key: Optional[str] = None):
        ...
        if OPENAI_AVAILABLE and openai_api_key and ai_service in ['openai', 'hybrid']:
            # Initialize OpenAI client with new v1.0+ interface
            ...
            if not OPENAI_AVAILABLE:
                ...
                logger.error("❌ No OpenAI API key provided - add OPENAI_API_KEY to secrets")
            elif ai_service == 'template':
                logger.error("❌ AI service configured for templates only - change to 'openai' in config")
            else:
                logger.error(f"❌ Invalid AI service configuration: {ai_service}")
            
            # Fail fast - no template fallbacks
            raise ValueError("AI generation is required but not properly configured. Check OpenAI API key and service configuration.")
    ...
    def _generate_with_ai(self, prompt: str, context: str = "", component_type: str = "content") -> str:
        """Generate content using OpenAI - no fallbacks, AI only"""
        if not self.ai_enabled or not self.openai_client:
            raise ValueError(f"AI generation required for {component_type} but not available")
        ...
        try:
            ...
            raise RuntimeError(f"Failed to generate {component_type} with AI: {e}")
    ...
    def generate_scripts(self, requirements: Dict, templates: Dict) -> Dict[str, str]:
        """Generate bash scripts using AI only"""
        logger.info("Generating bash scripts...")
        logger.info("🔧 Script generation method: AI")
...
'''
    ...
    def generate_robot_tasks(self, requirements: Dict, templates: Dict) -> str:
        """Generate Robot Framework tasks using AI only"""
        logger.info("Generating Robot Framework tasks...")
        logger.info("🤖 Robot tasks generation method: AI")
...
5. For automated monitoring and alerting in CI/CD pipelines'''
    ...
    def generate_readme(self, requirements: Dict) -> str:
        """Generate README.md content using AI only"""
        logger.info("Generating README.md...")
        logger.info("📚 Documentation generation method: AI")
