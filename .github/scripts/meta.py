    ...
    for runbook in runbook_files:
        ...
        for item in interesting_commands:
            ...
            if command.startswith('bash'):
                ...
                query_multi_line_with_comments_prompt = f"Add docstrings/comments to the following script. Return it in bash format without adding markdown codeblock formatting like ```bash```."
                ...
                if ((response_what_it_does.status_code == 200)):
                    ...
                    explanation_content = "Explanation not available"
                #Generate multi-line explanation 
                query_multi_line_with_comments_prompt = f"Convert this command to a multi-line command and add docstrings/comments. Return it in bash format without adding markdown codeblock formatting like ```bash```: "
                query_multi_line_with_command = f'{query_multi_line_with_comments_prompt}\n{command}'
                #Generate external doc links 
