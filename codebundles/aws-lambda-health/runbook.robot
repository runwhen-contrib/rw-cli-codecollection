...
*** Tasks ***
List AWS Lambda Versions and Runtimes in specified AWS Region
...
Analyze AWS Lambda Invocation Errors for Function `$${FUNCTION_NAME}` in AWS Region `$${AWS_REGION}`
...
Monitor AWS Lambda Performance Metrics such as duration, error count, and concurrency in AWS Region `${AWS_REGION}`
