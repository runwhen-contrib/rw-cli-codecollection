...
# =============================================================================
# Sourceable env file for local runbook testing
# =============================================================================

resource "local_file" "test_env" {
  filename = "${path.module}/../test-env.sh"
  content  = <<-EOF
    # Auto-generated by Terraform -- source this file to test the runbook locally.
    # Usage: source .test/test-env.sh
    export RW_MODE=dev
    export PYTHONPATH="${abspath("${path.module}/../../../../libraries")}:$${PYTHONPATH:-}"

    # Robot user variables
    export AWS_REGION="${var.aws_region}"
    export AWS_ACCOUNT_NAME="runwhen-test-${data.aws_caller_identity.current.account_id}"
    export SQS_QUEUE_URLS="${aws_sqs_queue.healthy_primary.url},${aws_sqs_queue.unhealthy_primary.url}"
    export RESOURCES="All"
    export DLQ_DEPTH_THRESHOLD="0"
    export CLOUDWATCH_LOG_LOOKBACK_MINUTES="60"
    export EXTRA_LOG_GROUP_NAMES=""
    export MAX_DLQ_SAMPLE_MESSAGES="5"

    # RW.Core secret configuration -- maps robot Import Secret names to providers.
    # aws:default@cli uses the default AWS credential chain (picks up env vars from tf.secret).
    export RW_SECRETS_KEYS='{"aws_credentials":"aws:default@cli"}'
  EOF
}
