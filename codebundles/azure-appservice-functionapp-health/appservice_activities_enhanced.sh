...
if ! resource_id=$(az functionapp show --name "$FUNCTION_APP_NAME" --resource-group "$AZ_RESOURCE_GROUP" --query "id" -o tsv 2>/dev/null); then        timestamp=$(date -u +"%Y-%m-%dT%H:%M:%S.%3NZ")
...
if [[ -z "$resource_id" ]]; then
    ...
    echo "Error: Empty resource ID returned for Function App $FUNCTION_APP_NAME. (detected at $log_timestamp)"
    issues_json=$(echo "$issues_json" | jq \
