...
if ! function_app_status=$(timeout 15s az functionapp show --name "$FUNCTION_APP_NAME" --resource-group "$AZ_RESOURCE_GROUP" --query "state" -o tsv 2>/dev/null); then        timestamp=$(date -u +"%Y-%m-%dT%H:%M:%S.%3NZ")
    ...
    echo "Error: Could not retrieve Function App status. Function App may not exist or access may be restricted. (detected at $log_timestamp)"
    exit 1
fi
