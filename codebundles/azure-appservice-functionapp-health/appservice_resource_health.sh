...
if [[ -z "$AZ_RESOURCE_GROUP" || -z "$FUNCTION_APP_NAME" ]]; then
    ...
    echo "Error: Please ensure AZ_RESOURCE_GROUP and FUNCTION_APP_NAME environment variables are set. (detected at $log_timestamp)"
    exit 1
fi
...
if [[ $? -eq 0 ]]; then
    ...
    echo "Error: Failed to retrieve health status for Azure Function App. (detected at $log_timestamp)"
    exit 1
fi
