...
if ! resource_id=$(timeout 10s az webapp show --name "$APP_SERVICE_NAME" --resource-group "$AZ_RESOURCE_GROUP" --query "id" -o tsv 2>/dev/null); then
    echo "Error: Could not retrieve App Service resource ID"
    issues_json=$(echo "$issues_json" | jq \
        ...
        if [[ -n "$failed_requests" && "$failed_requests" != "[]" && "$failed_requests" != "null" ]]; then
            ...
            if [[ $failed_count -gt 0 ]]; then
                ...
                issues_json=$(echo "$issues_json" | jq \
                    ...
                    --arg severity "2" \
                    --arg observed_at "$failed_timestamp" \
                    '.issues += [{"title": $title, "details": $details, "next_steps": $nextSteps, "severity": ($severity | tonumber), "observed_at": $observed_at}]')
            else
