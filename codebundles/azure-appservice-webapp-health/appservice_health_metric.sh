...
if ! resource_id=$(az webapp show --name "$APP_SERVICE_NAME" --resource-group "$AZ_RESOURCE_GROUP" --query "id" -o tsv 2>/dev/null); then        timestamp=$(date -u +"%Y-%m-%dT%H:%M:%S.%3NZ")
    ...
    echo "Error: App Service $APP_SERVICE_NAME not found in resource group $AZ_RESOURCE_GROUP. (detected at $log_timestamp)"
...
if [[ -z "$resource_id" ]]; then
    ...
    echo "Error: Empty resource ID returned for App Service $APP_SERVICE_NAME. (detected at $log_timestamp)"
    issues_json=$(echo "$issues_json" | jq \
