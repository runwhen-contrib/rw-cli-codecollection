...
echo "Retrieving health status for Azure Container App..."
healthUrl="https://management.azure.com/subscriptions/$subscription/resourceGroups/$AZ_RESOURCE_GROUP/providers/Microsoft.App/containerApps/$CONTAINER_APP_NAME/providers/Microsoft.ResourceHealth/availabilityStatuses/current?api-version=2023-07-01-preview"

az rest --method get --url "$healthUrl" > "container_app_health.json"

if [[ $? -eq 0 ]]; then
    echo "Health status retrieved successfully. Output saved to container_app_health.json"
    cat "container_app_health.json"
else
    echo "Error: Failed to retrieve health status for Azure Container App."
    exit 1
fi 