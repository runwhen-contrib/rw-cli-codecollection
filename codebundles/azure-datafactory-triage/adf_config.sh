...
resource_id=$(az datafactory show --resource-group $AZ_RESOURCE_GROUP --name $ADF --query id -o tsv)
echo "Scanning configuration of resource $resource_id"
prowler azure --az-cli-auth --service storage --output-directory /tmp/prowler --output-filename prowler_azure_scan > /dev/null
