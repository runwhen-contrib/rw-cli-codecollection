...
check_vm_memory() {
    ...
    status=$(echo "$memory_output" | jq -r '.value[0].displayStatus' 2>/dev/null)
    code=$(echo "$memory_output" | jq -r '.value[0].code' 2>/dev/null)
...
while read -r vm; do
    ...
    check_vm_memory "$vm_name" "$resource_group" "$temp_file" &
    pids+=($!)
    ((job_count++))
