...
Check If Kong Ingress HTTP Request Latency Violates Threshold
    ...
    ...    set_issue_details=The returned stdout line: $_line indicates there's high HTTPrequest latencies! You need to investigate the application associated with: ${INGRESS_SERVICE} or the Kong ingress controller. 
    ...    _line__raise_issue_if_contains=Route
    ${history}=    RW.CLI.Pop Shell History
    RW.Core.Add Pre To Report    Commands Used: ${history}
