#!/usr/bin/env bash
# -----------------------------------------------------------------------------
# Template render tier for gcp-apigee-environment-health.
#
# Renders the SLX and taskset templates through the same jinja2 configuration
# runwhen-local uses and asserts on the result. Complements .test/offline/run.sh,
# which greps the templates: grepping catches a known regression, rendering
# catches the class -- notably that reaching through an absent attribute RAISES
# under plain jinja2.Undefined rather than falling back.
#
