...
if [ -n "${unattached}" ]; then
    issue=$(jq -n \
        --arg title "Apigee environment groups have no attached environments in org \`${APIGEE_ORG}\`" \
...
if [ -n "${nohostname}" ]; then
    issue=$(jq -n \
        --arg title "Apigee environment groups have no routing hostnames in org \`${APIGEE_ORG}\`" \
