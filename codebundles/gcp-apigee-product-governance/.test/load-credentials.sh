...
# every accepted spelling into the two variables the scripts here actually use.
#
# Canonical location:  .test/terraform/tf.secret   (matches the sibling bundles)
# Also accepted:       .test/tf.secret             (this bundle's original path)
#
...
#
# The "organizations/" prefix is stripped: the API paths this bundle builds
...
if [ -z "$_lc_secret" ]; then
  ...
  echo "       See .test/README.md 'Manual steps'." >&2
  exit 1
fi

if [ "$_lc_secret" = "tf.secret" ]; then
  echo "note: using legacy .test/tf.secret; the sibling Apigee bundles expect .test/terraform/tf.secret" >&2
fi
