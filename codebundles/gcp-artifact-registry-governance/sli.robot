...
Suite Initialization
    ...
    # Establish the gcloud session once for the whole suite. `secret_file__`
    # materializes the credential inside the worker and GOOGLE_APPLICATION_CREDENTIALS
    # points at it. Keep the `|| true`: on the platform the secret import has
    # already logged in and the materialized value may be a status string rather
    # than a key, so this is a harmless no-op there and the real login in dev mode.
    RW.CLI.Run CLI
