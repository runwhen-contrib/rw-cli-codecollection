...
Fetch GCP Bucket Storage Utilization for `${PROJECT_IDS}`
    ...
    ...    cmd=cat ${RUNWHEN_TEMP_DIR}/bucket_report.json | jq '[.[] | select(.size_tb | tonumber > ${USAGE_THRESHOLD})] | length'
    ...    env=${env}
...
Check GCP Bucket Security Configuration for `${PROJECT_IDS}`
    ...
    ...    cmd=cat ${RUNWHEN_TEMP_DIR}/bucket_security_issues.json | jq . 
    ...    env=${env}
    ${total_public_access_buckets}=    RW.CLI.Run Cli
    ...    cmd=cat ${RUNWHEN_TEMP_DIR}/bucket_security_issues.json | jq '[.[] | select(.issue_type == "public_access")] | length' 
    ...    env=${env}
...
Fetch GCP Bucket Storage Operations Rate for `${PROJECT_IDS}`
    ...
    ...    cmd=cat ${RUNWHEN_TEMP_DIR}/bucket_ops_report.json | jq '[.[] | select(.total_ops | tonumber > ${OPS_RATE_THRESHOLD})] | length'
    ...    env=${env}
