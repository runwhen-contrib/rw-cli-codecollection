...
Score Cloud Composer Environment State in GCP Project `${GCP_PROJECT_ID}`
    ...
    ...    env=${env}
    ${count}=    Set Variable If    '${issues_output.stdout}' == ''    0    ${issues_output.stdout}
    ${state_score}=    Evaluate    1 if int(${count}) == 0 else 0
    Set Suite Variable    ${state_score}
    RW.Core.Push Metric    ${count}    sub_name=unhealthy_environment_count
    RW.Core.Push Metric    ${state_score}    sub_name=environment_state
...
Score Cloud Composer Configuration Health in GCP Project `${GCP_PROJECT_ID}`
    ...
    RW.Core.Push Metric    ${count}    sub_name=config_issue_count
...
Score Cloud Composer DAG and Scheduler Health in GCP Project `${GCP_PROJECT_ID}`
    ...
    RW.Core.Push Metric    ${count}    sub_name=jobs_scheduler_issue_count
...
Score Cloud Composer Worker and Queue Health in GCP Project `${GCP_PROJECT_ID}`
    ...
    RW.Core.Push Metric    ${count}    sub_name=workers_queues_issue_count
...
Score Cloud Composer Error Log Health in GCP Project `${GCP_PROJECT_ID}`
    ...
    RW.Core.Push Metric    ${count}    sub_name=error_log_environment_count
...
Generate Aggregate Cloud Composer Health Score for `${GCP_PROJECT_ID}`
    ...
    RW.Core.Add to Report    Cloud Composer Health Score: ${health_score} -- state: ${state_score}, config: ${config_score}, dag/scheduler: ${jobs_score}, workers/queues: ${worker_score}, error_logs: ${logs_score}
