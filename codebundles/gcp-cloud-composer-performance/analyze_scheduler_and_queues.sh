...
fetch cloud_composer_environment::composer.googleapis.com/environment/scheduler_heartbeat_count
| filter resource.environment_name == '${ENV_NAME}'
| within ${LOOKBACK_WINDOW_MINUTES}m
| group_by [resource.environment_name]
MQL
...
queue_max_r=$(printf '%.2f' "$queue_max")
queue_pct_above_r=$(printf '%.1f' "$queue_pct_above")

echo "=== Scheduler and Queue Analysis: ${ENV_NAME} ==="
echo "Lookback window: ${LOOKBACK_WINDOW_MINUTES} minutes"
echo "Queue backlog threshold: ${QUEUE_BACKLOG_THRESHOLD} tasks (flagged if average at/above threshold, or above for >= ${QUEUE_BACKLOG_PERCENT_OF_TIME}% of the window)"
echo "Scheduler heartbeat minimum: ${SCHEDULER_HEARTBEAT_MIN} (0 = check disabled)"
echo ""
echo "Task queue depth (task_queue_length):"
echo "  average: ${queue_avg_r}"
echo "  maximum: ${queue_max_r}"
echo "  samples: ${queue_count}"
echo "  time above ${QUEUE_BACKLOG_THRESHOLD} threshold: ${queue_pct_above_r}%"
echo ""
...
if [ "$queue_count" -gt 0 ]; then
    ...
    if [ "$backlog" = "true" ]; then
        ...
        issues=$(add_issue \
            ...
            "The task queue for environment '${ENV_NAME}' averaged ${queue_avg_r} (max ${queue_max_r}) over the last ${LOOKBACK_WINDOW_MINUTES}m, above the ${QUEUE_BACKLOG_THRESHOLD} backlog threshold ${queue_pct_above_r}% of the time. A persistent backlog indicates scheduler/worker capacity cannot keep up with generated tasks." \
            "3" \
            "The task queue should be consistently drained below the ${QUEUE_BACKLOG_THRESHOLD} depth threshold." \
            "Task queue averaged ${queue_avg_r} with max ${queue_max_r} over the lookback window." \
...
if [ "${SCHEDULER_HEARTBEAT_MIN}" != "0" ]; then
    if [ "$heartbeat_total" -lt "$SCHEDULER_HEARTBEAT_MIN" ]; then
        verdict="${verdict}, SCHEDULER STALLED"
        issues=$(add_issue \
            "$issues" \
...
echo "Scheduler and queue analysis complete: $(jq length "$OUTPUT_FILE") issue(s) found."