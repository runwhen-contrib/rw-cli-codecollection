...
if ! command -v gcloud &>/dev/null; then
  ...
  echo "Error: gcloud not found on PATH. (detected at $log_timestamp)" >&2
  exit 1
fi
if ! command -v jq &>/dev/null; then
  ...
  echo "Error: jq not found on PATH. (detected at $log_timestamp)" >&2
  exit 1
fi
...
if [ -z "$PROJECT" ]; then
  ...
  echo "Error: No project set. Use GCP_PROJECT_ID or 'gcloud config set project <PROJECT_ID>'. (detected at $log_timestamp)" >&2
  exit 1
fi
