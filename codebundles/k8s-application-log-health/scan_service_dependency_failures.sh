...
ISSUES_JSON='{"issues": []}'
ISSUES_OUTPUT=${OUTPUT_DIR}/scan_service_issues.json
ERROR_COUNTS=()
declare -A ERROR_STATS

echo "Scanning logs for service dependency failures in ${WORKLOAD_TYPE}/${WORKLOAD_NAME} in namespace ${NAMESPACE}..."
...
for POD in ${PODS[@]}; do
    ...
    for CONTAINER in ${CONTAINERS}; do
        ...
        ERROR_FILE="${OUTPUT_DIR}/${POD}_${CONTAINER}_service_failures.txt"
        ...
        for pattern in "${!ERROR_AGGREGATE[@]}"; do
            ...
            if ((OCCURRENCES >= 10)); then
                NEXT_STEP+=" Multiple failures detected (${OCCURRENCES} occurrences). Immediate troubleshooting recommended."
                SEVERITY=1
            elif ((OCCURRENCES >= 5)); then
                NEXT_STEP+=" Repeated failures detected (${OCCURRENCES} occurrences). Consider checking service dependencies and retries."
                SEVERITY=2
            fi

            ISSUES_JSON=$(echo "$ISSUES_JSON" | jq \
                --arg title "Service Dependency Failure Detected in ${POD} (${CONTAINER}) - ${CATEGORY}" \
                --arg details "${DETAILS}" \
...
if [[ ${#ERROR_COUNTS[@]} -gt 0 ]]; then
    ...
    SUMMARY="Detected service dependency failures affecting ${UNIQUE_CATEGORIES}."
else
    SUMMARY="No service dependency failures detected."
fi
