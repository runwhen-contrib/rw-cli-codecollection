...
ISSUES_JSON='{"issues": []}'
ISSUES_OUTPUT=${OUTPUT_DIR}/scan_timeout_issues.json

ERROR_COUNTS=()
declare -A ERROR_STATS

echo "Scanning logs for timeout errors in ${WORKLOAD_TYPE}/${WORKLOAD_NAME} in namespace ${NAMESPACE}..."
...
for POD in ${PODS[@]}; do
    ...
    for CONTAINER in ${CONTAINERS}; do
        ...
        ERROR_FILE="${OUTPUT_DIR}/${POD}_${CONTAINER}_timeout_errors.txt"
        ...
        done < <(jq -r '.patterns[] | select(.category == "Timeout") | .match' "${ERROR_JSON}")
        ...
        for pattern in "${!ERROR_AGGREGATE[@]}"; do
            ...
            ISSUES_JSON=$(echo "$ISSUES_JSON" | jq \
                --arg title "Timeout Error Detected in ${POD} (${CONTAINER}) - ${CATEGORY}" \
                --arg details "${DETAILS}" \
...
if [[ ${#ERROR_COUNTS[@]} -gt 0 ]]; then
    ...
    SUMMARY="Detected timeout issues affecting ${UNIQUE_CATEGORIES}."
else
    SUMMARY="No timeout errors detected."
fi
