...
# Set daemonset name and namespace
...
# Get daemonset manifest in JSON format
MANIFEST=$(${KUBERNETES_DISTRIBUTION_BINARY} get daemonset "$DAEMONSET_NAME" -n "$NAMESPACE" --context "$CONTEXT" -o json)
if [ $? -ne 0 ]; then
    echo "Error fetching daemonset details: $MANIFEST"
    exit 1
fi
...
if [ -z "$NUM_CONTAINERS" ]; then
    echo "No containers found in daemonset."
    exit 1
fi
...
for ((i=0; i<NUM_CONTAINERS; i++)); do
    ...
    if echo "$PROBE" | jq -e '.httpGet, .tcpSocket' >/dev/null; then
        ...
        if [[ ! " $CONTAINER_PORTS " == *"$PROBE_PORT"* ]]; then
            ...
            next_steps+=("Update $PROBE_TYPE For \`${DAEMONSET_NAME}\` to use one of the following ports: $CONTAINER_PORTS")
        else
    ...
    if echo "$PROBE" | jq -e '.exec' >/dev/null; then
        ...
        if [ -n "$PORT_IN_COMMAND" ]; then
            ...
            if [[ ! " $CONTAINER_PORTS " == *"$PORT_IN_COMMAND"* ]]; then
                ...
                next_steps+=("Get DaemonSet Workload Details For \`${DAEMONSET_NAME}\`")
                next_steps+=("Verify and Reconfigure Manifest $PROBE_TYPE with Valid Ports For \`${DAEMONSET_NAME}\`")
            else
        ...
        if ${KUBERNETES_DISTRIBUTION_BINARY} auth can-i create pods/exec -n "$NAMESPACE" >/dev/null 2>&1; then
            POD_NAME=$(${KUBERNETES_DISTRIBUTION_BINARY} get pods -n "$NAMESPACE" -l "app=$DAEMONSET_NAME" -o jsonpath="{.items[0].metadata.name}")
            if [ -z "$POD_NAME" ]; then
                echo "No pods found for daemonset $DAEMONSET_NAME."
                continue
            fi
            ...
            if [[ -n "$CONTAINER_PORTS" && "$EXEC_EXIT_CODE" != 0 ]]; then
                ...
                # Execute modified command
                    ...
                    if [ $EXEC_EXIT_CODE == 0 ]; then
                        next_steps+=("Update $PROBE_TYPE For \`${DAEMONSET_NAME}\` to use port $PORT")
                    fi
                    printf "### END Exec Test\n"
                done
            fi
