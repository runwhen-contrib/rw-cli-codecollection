...
for ((o=0; o<NUM_OBJECTS; o++)); do
    ...
    if [[ "$HAS_TOO_MANY_REPLICAS" != "NO" ]]; then
        ...
        remediation_steps+=("{\"remediation_type\":\"increase_hpa_replicas\",\"object_type\":\"HorizontalPodAutoscaler\",\"object_name\":\"$OBJECT_NAME\", \"max_replicas\":\"${MAX_REPLICAS}\", \"new_max_replicas\":\"${NEW_MAX_REPLICAS}\"}")
    fi

    METRICS=$(extract_data "$HPA_OBJECTS" ".items[$o].spec.metrics")
    METRIC_NAME=$(extract_data "$HPA_OBJECTS" ".items[$o].spec.metrics[0].resource.name")
    METRIC_TARGET=$(extract_data "$HPA_OBJECTS" ".items[$o].spec.metrics[0].resource.target")
    METRIC_UTIL_VALUE=$(extract_data "$HPA_OBJECTS" ".items[$o].spec.metrics[0].resource.target.averageUtilization")
    METRICS_CURRENT=$(extract_data "$HPA_OBJECTS" ".items[$o].status.currentMetrics[0].resource.current")

    TARET_REF=$(extract_data "$HPA_OBJECTS" ".items[$o].spec.scaleTargetRef")
    TARGET_KIND=$(extract_data "$HPA_OBJECTS" ".items[$o].spec.scaleTargetRef.kind")
    TARGET_NAME=$(extract_data "$HPA_OBJECTS" ".items[$o].spec.scaleTargetRef.name")

    echo "-------- START Validation -------"
    echo "Object: HorizontalPodAutoscaler/$OBJECT_NAME"
    echo "Metric Name: $METRIC_NAME"
    echo "Metric Target: $METRIC_TARGET"
    echo "Current Metric: $METRICS_CURRENT"
