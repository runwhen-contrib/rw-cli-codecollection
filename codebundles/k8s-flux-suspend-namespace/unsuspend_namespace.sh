...
for NAMESPACE_WORKLOAD in $NAMESPACE_WORKLOADS; do
    manifest_json=$(kubectl get $NAMESPACE_WORKLOAD -n $NAMESPACE -o json)
    ...
    flux_object_manifest=$(kubectl get -n $NAMESPACE kustomizations.kustomize.toolkit.fluxcd.io/$flux_object -ojson)
