...
calculate_recommendation() {
    ...
    # Convert size to a number in GB (assuming size is in the format of '10G' or '10.0G')
    size_in_gb=$(echo $current_size | sed 's/G//')

    # Ensure the size is a number (allowing decimal points)
...
for pod in $(${KUBERNETES_DISTRIBUTION_BINARY} get pods -n ${NAMESPACE} --context ${CONTEXT} -o json | jq -r '.items[] | select(.spec.volumes[]?.persistentVolumeClaim) | select(.status.phase=="Running") | .metadata.name'); do
    ...
    for pvc in $(echo "$pod_json" | jq -r '.spec.volumes[] | select(has("persistentVolumeClaim")) | .persistentVolumeClaim.claimName'); do
        ...
        disk_size=$(${KUBERNETES_DISTRIBUTION_BINARY} exec $pod -n ${NAMESPACE} --context ${CONTEXT} -c $containerName -- df -h $mountPath | awk 'NR==2 {print $2}')
        ...
        if [ $recommended_new_size -ne 0 ]; then
            ...
            recommendation="{ \"remediation_type\":\"pvc_increase\", \"object_type\":\"PersistentVolumeClaim\", \"object_name\":\"$pvc\", \"pod\": \"$pod\", \"volume_name\": \"$volumeName\", \"container_name\": \"$containerName\", \"mount_path\": \"$mountPath\", \"current_size\": \"$disk_size\", \"usage\": \"$disk_usage%\", \"recommended_size\": \"${recommended_new_size}G\", \"severity\": \"$severity\" }"
