...
*** Tasks ***
Search For GCE Ingress Warnings in GKE Cluster '$${CONTEXT}'
    ...
    ...    set_issue_next_steps=Validate GCP HTTP Load Balancer Configurations in Kubernetes Cluster `$${NAMESPACE}` using Context `$${CONTEXT}` for ${INGRESS}
    ...    _line__raise_issue_if_contains=Warning
    ${history}=    RW.CLI.Pop Shell History
    RW.Core.Add Pre To Report    GCE Ingress warnings for ${NAMESPACE}:\n\n${event_warnings.stdout}
    RW.Core.Add Pre To Report    Commands Used: ${history}

Identify Unhealthy GCE HTTP Ingress Backends in Kubernetes Cluster $${CONTEXT} and Namespace $${NAMESPACE}
...
Validate GCP HTTP Load Balancer Configurations in Kubernetes Cluster `$${NAMESPACE}` using Context `$${CONTEXT}`
...
Fetch HTTP Load Balancer Error Logs from GCP Operations Manager for specific Ingress Backends within the last 60 minutes
...
Create URLs for obtaining GCP Operations Logs from the Logging Dashboard
