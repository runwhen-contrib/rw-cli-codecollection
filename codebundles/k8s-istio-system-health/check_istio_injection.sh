...
function check_cluster_connection() {
    ...
    if ! "${KUBERNETES_DISTRIBUTION_BINARY}" cluster-info --context="${CONTEXT}" 2>&1 >/dev/null; then
        echo "=== Cluster Info ==="
        "${KUBERNETES_DISTRIBUTION_BINARY}" cluster-info --context="${CONTEXT}"        timestamp=$(date -u +"%Y-%m-%dT%H:%M:%S.%3NZ")
        ...
        echo "Error: Unable to connect to the cluster. Please check your kubeconfig and cluster status. (detected at $log_timestamp)"
        exit 1
    fi
    
    # Check API server availability
    if ! "${KUBERNETES_DISTRIBUTION_BINARY}" get --raw="/api" --context="${CONTEXT}" 2>&1 >/dev/null; then        timestamp=$(date -u +"%Y-%m-%dT%H:%M:%S.%3NZ")
...
function check_jq_error() {
    if [ $? -ne 0 ]; then
        ...
        echo "Error: Failed to parse JSON output (detected at $log_timestamp)"
        exit 1
    fi
}
...
if [ $? -ne 0 ]; then
    ...
    echo "Error: Failed to get namespaces. Please check your permissions and cluster status. (detected at $log_timestamp)"
    exit 1
fi
...
if [ -z "$FILTERED_NAMESPACES" ]; then
    ...
    echo "Error: No namespaces found (excluding: ${EXCLUDED_NAMESPACES}) (detected at $log_timestamp)"
    exit 1
fi
...
for ns in $FILTERED_NAMESPACES; do
    ...
    if [ $? -ne 0 ]; then
        ...
        echo "Error: Failed to get namespace $ns details. Skipping... (detected at $log_timestamp)"
        continue
    fi
    ...
    if [ $? -ne 0 ]; then
        ...
        echo "Error: Failed to get deployments in namespace $ns. Skipping... (detected at $log_timestamp)"
        continue
    fi
    ...
    for deployment in "${DEPLOYMENT_NAMES[@]}"; do
        ...
        timestamp=$(date -u +"%Y-%m-%dT%H:%M:%S.%3NZ")
                        ...
                        echo "Error: Failed to get pods for deployment $deployment. Add app label or canonical-name. Skipping... (detected at $log_timestamp)"
                        continue
                    fi
                fi
            fi
