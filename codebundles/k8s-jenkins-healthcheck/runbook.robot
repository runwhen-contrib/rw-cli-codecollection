...
Query For Stuck Jenkins Jobs
    ...
    ${rsp}=    RW.CLI.Run Cli
    ...    cmd=${KUBERNETES_DISTRIBUTION_BINARY} exec statefulset/${STATEFULSET_NAME} --context=${CONTEXT} -n ${NAMESPACE} -- curl -s localhost:8080/queue/api/json --user $${JENKINS_SA_USERNAME.key}:$${JENKINS_SA_TOKEN.key} | jq -r '.items[] | select((.stuck == false) or (.blocked == false)) | "Why: " + .why + "\nBlocked: " + (.blocked|tostring) + "\nStuck: " + (.stuck|tostring)'
    ...    secret__jenkins_sa_username=${JENKINS_SA_USERNAME}
    ...    secret__jenkins_sa_token=${JENKINS_SA_TOKEN}
    ...    render_in_commandlist=true
    ...    env=${env}
