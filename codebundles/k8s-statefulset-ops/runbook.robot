*** Settings ***
Documentation       Provides StatefulSet-scoped operational tasks including restart, pod recycle, rollback, scaling, HPA, and resource tuning.
Metadata            Author    rw-codebundle-agent
Metadata            Display Name    Kubernetes StatefulSet Operations
Metadata            Supports    Kubernetes StatefulSet Operations AKS EKS GKE OpenShift
...
Restart StatefulSet `${STATEFULSET_NAME}` in Namespace `${NAMESPACE}`
    [Documentation]    Perform a rollout restart on the StatefulSet with optional rollout status wait; failures surface as issues.
    [Tags]
    ...    log
    ...    pod
    ...    restart
    ...
    ${stub}=    RW.CLI.Run Bash File
    ...    bash_file=emit-issue-json-stub.sh
    ...    env=${env}
    ...    include_in_history=false
    ...    timeout_seconds=180
    ...    show_in_rwl_cheatsheet=true
    ...    cmd_override=./emit-issue-json-stub.sh
    ...    secret_file__kubeconfig=${kubeconfig}
    TRY
        ${stub_issues}=    RW.CLI.Run Cli
        ...    cmd=cat issue_stub_output.json
        ...    env=${env}
        ...    timeout_seconds=60
        ...    secret_file__kubeconfig=${kubeconfig}
        ${issue_list}=    Evaluate    json.loads(r'''${stub_issues.stdout}''')    json
    EXCEPT
        Log    Stub JSON parse skipped    WARN
    END
...
Force Delete Pods in StatefulSet `${STATEFULSET_NAME}` in Namespace `${NAMESPACE}`
    ...
    ...    cmd=${KUBERNETES_DISTRIBUTION_BINARY} delete pods -n ${NAMESPACE} --context ${CONTEXT} -l $(kubectl get statefulset ${STATEFULSET_NAME} -n ${NAMESPACE} --context ${CONTEXT} -o jsonpath='{.spec.template.metadata.labels}' | tr -d '{}" ' | tr ':' '=')
    ...    env=${env}
    ...    include_in_history=false
    ...    timeout_seconds=180
    ...    secret_file__kubeconfig=${kubeconfig}
    ...
    IF    ($force_delete.stderr) == ""
        ...
        ...    cmd=${KUBERNETES_DISTRIBUTION_BINARY} get pods -n ${NAMESPACE} --context ${CONTEXT} -l $(kubectl get statefulset ${STATEFULSET_NAME} -n ${NAMESPACE} --context ${CONTEXT} -o jsonpath='{.spec.template.metadata.labels}' | tr -d '{}" ' | tr ':' '=')
        ...    env=${env}
        ...    include_in_history=false
        ...    timeout_seconds=180
        ...    secret_file__kubeconfig=${kubeconfig}
...
Scale Up StatefulSet `${STATEFULSET_NAME}` in Namespace `${NAMESPACE}` by ${SCALE_UP_FACTOR}x
    [Documentation]    Multiply current StatefulSet replicas by SCALE_UP_FACTOR capped by MAX_REPLICAS.
    [Tags]
    ...    scaleup
    ...    statefulset
    ...    ${STATEFULSET_NAME}
...
Suite Initialization
    ...
    Set Suite Variable
    ...    ${env}
    ...    {"KUBECONFIG":"./${kubeconfig.key}", "KUBERNETES_DISTRIBUTION_BINARY":"${KUBERNETES_DISTRIBUTION_BINARY}", "CONTEXT":"${CONTEXT}", "NAMESPACE":"${NAMESPACE}", "STATEFULSET_NAME": "${STATEFULSET_NAME}", "SCALE_UP_FACTOR":"${SCALE_UP_FACTOR}", "MAX_REPLICAS": "${MAX_REPLICAS}", "ALLOW_SCALE_TO_ZERO":"${ALLOW_SCALE_TO_ZERO}", "HPA_SCALE_FACTOR":"${HPA_SCALE_FACTOR}", "HPA_MAX_REPLICAS":"${HPA_MAX_REPLICAS}", "HPA_MIN_REPLICAS":"${HPA_MIN_REPLICAS}", "RESOURCE_SCALE_DOWN_FACTOR":"${RESOURCE_SCALE_DOWN_FACTOR}"}
