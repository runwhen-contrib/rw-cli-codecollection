...
Metadata            Supports    Kubernetes,AKS,EKS,GKE,OpenShift,StatefulSet
...
Restart StatefulSet `${STATEFULSET_NAME}` in Namespace `${NAMESPACE}`
    ...
    TRY
        Log    Restart task started for StatefulSet ${STATEFULSET_NAME}    INFO
    EXCEPT
        Log    Task preamble logging failed (non-fatal)    WARN
    END
    ...
    IF    ($rollout.stderr) == ""
        ...
        ...    cmd=while true; do STATUS=$(${KUBERNETES_DISTRIBUTION_BINARY} rollout status statefulset/${STATEFULSET_NAME} -n ${NAMESPACE} --context ${CONTEXT} --timeout=180s); echo "$STATUS"; ([[ "$STATUS" == *"successfully rolled out"* ]] || [[ "$STATUS" == *"roll out complete"* ]] || [[ "$STATUS" == *"rolling update complete"* ]]) && break; sleep 5; done
        ...    env=${env}
        ...    include_in_history=false
        ...    timeout_seconds=180
        ...    secret_file__kubeconfig=${kubeconfig}
...
Increase CPU Resources for StatefulSet `${STATEFULSET_NAME}` in Namespace `${NAMESPACE}`
    ...
    ...    cmd=${KUBERNETES_DISTRIBUTION_BINARY} get vpa -n ${NAMESPACE} --context ${CONTEXT} -o json | jq -r '.items[] | select(.spec.targetRef.name=="${STATEFULSET_NAME}" and (.spec.targetRef.kind=="StatefulSet" or .spec.targetRef.kind=="statefulset" or .spec.targetRef.kind==null)) | .status.recommendation.containerRecommendations[0].upperBound.cpu' 2>/dev/null || echo ""
    ...    env=${env}
    ...    include_in_history=true
    ...    timeout_seconds=180
    ...    secret_file__kubeconfig=${kubeconfig}
...
Increase Memory Resources for StatefulSet `${STATEFULSET_NAME}` in Namespace `${NAMESPACE}`
    ...
    # Check if VPA exists with recommendations
    ${vpa_check}=    RW.CLI.Run Cli
    ...    cmd=${KUBERNETES_DISTRIBUTION_BINARY} get vpa -n ${NAMESPACE} --context ${CONTEXT} -o json | jq -r '.items[] | select(.spec.targetRef.name=="${STATEFULSET_NAME}" and (.spec.targetRef.kind=="StatefulSet" or .spec.targetRef.kind=="statefulset" or .spec.targetRef.kind==null)) | .status.recommendation.containerRecommendations[0].upperBound.memory' 2>/dev/null || echo ""
    ...    env=${env}
    ...    include_in_history=true
    ...    timeout_seconds=180
    ...    secret_file__kubeconfig=${kubeconfig}
...
Suite Initialization
    ...
    ...    type=string
    ...    description=The multiple in which to increase the total amount of pods. For example, a StatefulSet with 2 pods and a scale up factor of 2 will result in 4 pods. 
    ...    example=2
    ...    default=2
    ...
    TRY
        ...
        ...    secret_file__kubeconfig=${kubeconfig}
    EXCEPT
        Log    Cluster connectivity verification failed; downstream tasks may still be attempted.    WARN
    END
