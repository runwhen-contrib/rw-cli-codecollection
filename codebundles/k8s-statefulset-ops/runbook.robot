*** Settings ***
Documentation       Perform operational tasks for a Kubernetes StatefulSet.
Metadata            Author    rw-codebundle-agent
Metadata            Display Name    Kubernetes StatefulSet Operations
Metadata            Supports    Kubernetes,AKS,EKS,GKE,OpenShift

Force Tags          Kubernetes    StatefulSet    Operations    k8s
...
Restart StatefulSet `${STATEFULSET_NAME}` in Namespace `${NAMESPACE}`
    [Documentation]    Rollout restart with pre-restart logs and rollout status wait; raises issues on failure.
    [Tags]
    ...    log
    ...    pod
    ...    restart
    ...
    ${vf}=    RW.CLI.Run Bash File
    ...    bash_file=verify-statefulset-access.sh
    ...    env=${env}
    ...    secret_file__kubeconfig=${kubeconfig}
    ...    timeout_seconds=180
    ...    include_in_history=false
    ...    show_in_rwl_cheatsheet=true
    ...    cmd_override=./verify-statefulset-access.sh

    RW.Core.Add Pre To Report    ----------\nVerify output:\n${vf.stdout}

    ${issues_raw}=    RW.CLI.Run Cli
    ...    cmd=cat verify_statefulset_issues.json
    ...    env=${env}
    ...    include_in_history=false
    ...    timeout_seconds=180
    ...    secret_file__kubeconfig=${kubeconfig}
    ...
    EXCEPT
        Log    Failed to parse verify JSON; assuming no issues.    WARN
    ...
    IF    ${n} > 0
        FOR    ${issue}    IN    @{issue_list}
            ...
            ...    expected=StatefulSet `${STATEFULSET_NAME}` should be reachable in namespace `${NAMESPACE}`
            ...    actual=StatefulSet `${STATEFULSET_NAME}` was not found or is not accessible
            ...    title=${issue}[title]
            ...    reproduce_hint=./verify-statefulset-access.sh
    ...
    ELSE
        ${logs}=    RW.CLI.Run Cli
        ...    cmd=${KUBERNETES_DISTRIBUTION_BINARY} logs statefulset/${STATEFULSET_NAME} --tail 50 --all-containers=true --max-log-requests=20 -n ${NAMESPACE} --context ${CONTEXT}
        ...    env=${env}
        ...
        ...    cmd=${KUBERNETES_DISTRIBUTION_BINARY} rollout restart statefulset/${STATEFULSET_NAME} -n ${NAMESPACE} --context ${CONTEXT}
        ...    env=${env}
        ...    include_in_history=false
        ...    timeout_seconds=180
        ...    secret_file__kubeconfig=${kubeconfig}
        ...
        IF    ($rollout.stderr) == ""
            ...
            ...    cmd=${KUBERNETES_DISTRIBUTION_BINARY} rollout status statefulset/${STATEFULSET_NAME} -n ${NAMESPACE} --context ${CONTEXT} --timeout=180s
            ...    env=${env}
            ...    include_in_history=false
            ...    timeout_seconds=180
            ...    secret_file__kubeconfig=${kubeconfig}
        ...
        IF    ($rollout.stderr) != ""
            RW.Core.Add Issue
            ...    severity=3
            ...    expected=StatefulSet `${STATEFULSET_NAME}` in namespace `${NAMESPACE}` should have rolled out successfully
            ...    actual=StatefulSet `${STATEFULSET_NAME}` in namespace `${NAMESPACE}` did not rollout successfully
            ...    title=StatefulSet `${STATEFULSET_NAME}` in `${NAMESPACE}` did not rollout properly
            ...    reproduce_hint=View Commands Used in Report Output
            ...    details=StatefulSet ${STATEFULSET_NAME} in Namespace ${NAMESPACE} generated the following output during restart attempt: \n${rollout.stderr}
            ...    next_steps=Inspect StatefulSet warning events for `${STATEFULSET_NAME}`\nEscalate rollout issues to service owner.
            ...    observed_at=${timestamp}
        END
        ${history}=    RW.CLI.Pop Shell History
    END
...
Force Delete Pods in StatefulSet `${STATEFULSET_NAME}` in Namespace `${NAMESPACE}`
    [Documentation]    Force delete pods selected via the StatefulSet pod template labels to recover stuck workloads.
    [Tags]
    ...    log
    ...    pod
    ...    restart
    ...
    ${logs}=    RW.CLI.Run Cli
    ...    cmd=${KUBERNETES_DISTRIBUTION_BINARY} logs statefulset/${STATEFULSET_NAME} --tail 50 -n ${NAMESPACE} --all-containers=true --max-log-requests=20 --context ${CONTEXT}
    ...    env=${env}
    ...    include_in_history=true
    ...    timeout_seconds=180
    ...    secret_file__kubeconfig=${kubeconfig}
    ...
    ...    cmd=${KUBERNETES_DISTRIBUTION_BINARY} delete pods -n ${NAMESPACE} --context ${CONTEXT} -l $(${KUBERNETES_DISTRIBUTION_BINARY} get statefulset ${STATEFULSET_NAME} -n ${NAMESPACE} --context ${CONTEXT} -o jsonpath='{.spec.template.metadata.labels}' | tr -d '{}" ' | tr ':' '=')
    ...    env=${env}
    ...    include_in_history=false
    ...    timeout_seconds=180
    ...    secret_file__kubeconfig=${kubeconfig}
    ...
    IF    ($force_delete.stderr) == ""
        ...
        ...    cmd=${KUBERNETES_DISTRIBUTION_BINARY} get pods -n ${NAMESPACE} --context ${CONTEXT} -l $(${KUBERNETES_DISTRIBUTION_BINARY} get statefulset ${STATEFULSET_NAME} -n ${NAMESPACE} --context ${CONTEXT} -o jsonpath='{.spec.template.metadata.labels}' | tr -d '{}" ' | tr ':' '=')
        ...    env=${env}
        ...    include_in_history=false
        ...    timeout_seconds=180
        ...    secret_file__kubeconfig=${kubeconfig}
    ...
    IF    ($force_delete.stderr) != ""
        RW.Core.Add Issue
        ...    severity=3
        ...    expected=StatefulSet `${STATEFULSET_NAME}` in namespace `${NAMESPACE}` should have force deleted pods successfully
        ...    actual=StatefulSet `${STATEFULSET_NAME}` in namespace `${NAMESPACE}` did not force deleted pods successfully
        ...    title=StatefulSet `${STATEFULSET_NAME}` in `${NAMESPACE}` did not force deleted pods successfully
        ...    reproduce_hint=View Commands Used in Report Output
        ...    details=StatefulSet ${STATEFULSET_NAME} in Namespace ${NAMESPACE} generated the following output during force deletion attempt: \n${force_delete.stderr}
...
Rollback StatefulSet `${STATEFULSET_NAME}` in Namespace `${NAMESPACE}` to Previous Version
    [Documentation]    kubectl rollout undo for the StatefulSet with status polling.
    [Tags]
    ...    log
    ...    pod
    ...    rollback
    ...
    ...    cmd=${KUBERNETES_DISTRIBUTION_BINARY} rollout undo statefulset/${STATEFULSET_NAME} -n ${NAMESPACE} --context ${CONTEXT}
    ...    env=${env}
    ...    include_in_history=false
    ...    timeout_seconds=180
    ...    secret_file__kubeconfig=${kubeconfig}
    ...
    IF    ($rollback.stderr) != ""
        RW.Core.Add Issue
        ...    severity=3
        ...    expected=StatefulSet `${STATEFULSET_NAME}` in namespace `${NAMESPACE}` should have rollback successfully
        ...    actual=StatefulSet `${STATEFULSET_NAME}` in namespace `${NAMESPACE}` did not rollback successfully
        ...    title=StatefulSet `${STATEFULSET_NAME}` in `${NAMESPACE}` did not rollback properly
        ...    reproduce_hint=View Commands Used in Report Output
        ...    details=StatefulSet ${STATEFULSET_NAME} in Namespace ${NAMESPACE} generated the following output during rollback attempt: \n${rollback.stderr}
...
Scale Down StatefulSet `${STATEFULSET_NAME}` in Namespace `${NAMESPACE}`
    [Documentation]    Scales replicas to 0 or 1 per ALLOW_SCALE_TO_ZERO; surfaces policy issue when zero is blocked. to immediately halt a failing or runaway service.
    [Tags]
    ...    log
    ...    pod
    ...    scaledown
    ...
    ELSE
        ...
        RW.Core.Add Issue
        ...    severity=4
        ...    expected=StatefulSet `${STATEFULSET_NAME}` in namespace `${NAMESPACE}` can not scale to 0
        ...    actual=StatefulSet `${STATEFULSET_NAME}` in namespace `${NAMESPACE}` can not scale to 0
        ...    title=StatefulSet `${STATEFULSET_NAME}` in `${NAMESPACE}` is not permitted to scale to 0
        ...    reproduce_hint=View Commands Used in Report Output
        ...    details=StatefulSet ${STATEFULSET_NAME} in Namespace ${NAMESPACE} is not permitted to scale to 0 due to CodeBundle configuration
        ...    next_steps=Reconfigure Codebundle if 0 replicas is desired. 
        ...    observed_at=${timestamp}
    END

    ${scaledown}=    RW.CLI.Run Cli
    ...    cmd=${KUBERNETES_DISTRIBUTION_BINARY} scale statefulset/${STATEFULSET_NAME} -n ${NAMESPACE} --context ${CONTEXT} --replicas=${desired_replicas}
    ...    env=${env}
    ...    include_in_history=true
    ...    timeout_seconds=180
    ...    secret_file__kubeconfig=${kubeconfig}
    ...
    ...    cmd=${KUBERNETES_DISTRIBUTION_BINARY} get statefulset ${STATEFULSET_NAME} -n ${NAMESPACE} --context ${CONTEXT} 
    ...    env=${env}
    ...    include_in_history=true
    ...    secret_file__kubeconfig=${kubeconfig}
    ...
    IF    ($scaledown.stderr) != ""
        RW.Core.Add Issue
        ...    severity=3
        ...    expected=StatefulSet `${STATEFULSET_NAME}` in namespace `${NAMESPACE}` should scale down successfully
        ...    actual=StatefulSet `${STATEFULSET_NAME}` in namespace `${NAMESPACE}` did not scale down successfully
        ...    title=StatefulSet `${STATEFULSET_NAME}` in `${NAMESPACE}` did not scale down properly
        ...    reproduce_hint=View Commands Used in Report Output
        ...    details=StatefulSet ${STATEFULSET_NAME} in Namespace ${NAMESPACE} generated the following output during scaledown attempt: \n${scaledown.stderr}
        ...    next_steps=Inspect StatefulSet warning Events for `${STATEFULSET_NAME}`\nEscalate rollback issues to service owner.
        ...    observed_at=${timestamp}
    END
...
Scale Up StatefulSet `${STATEFULSET_NAME}` in Namespace `${NAMESPACE}` by ${SCALE_UP_FACTOR}x
    [Documentation]    Increase StatefulSet replicas 
    [Tags]
    ...    scaleup
    ...    statefulset
    ...    ${STATEFULSET_NAME}
    ...
    ...    cmd=${KUBERNETES_DISTRIBUTION_BINARY} get statefulset ${STATEFULSET_NAME} -n ${NAMESPACE} --context ${CONTEXT} -o jsonpath='{.spec.replicas}'
    ...    secret_file__kubeconfig=${kubeconfig}
    ...    include_in_history=True
    ...    timeout_seconds=180
    ...    env=${env}
    ...
    IF     ${scaled} <= ${MAX_REPLICAS}
        ...
        ...    cmd=${KUBERNETES_DISTRIBUTION_BINARY} scale statefulset/${STATEFULSET_NAME} -n ${NAMESPACE} --context ${CONTEXT} --replicas=${scaled}
        ...    env=${env}
        ...    include_in_history=true
        ...    timeout_seconds=180
        ...    secret_file__kubeconfig=${kubeconfig}
        ${replicas}=    RW.CLI.Run Cli
        ...    cmd=${KUBERNETES_DISTRIBUTION_BINARY} get statefulset ${STATEFULSET_NAME} -n ${NAMESPACE} --context ${CONTEXT} && ${KUBERNETES_DISTRIBUTION_BINARY} get statefulset ${STATEFULSET_NAME} -n ${NAMESPACE} --context ${CONTEXT} -o jsonpath='{.spec.replicas}'
        ...    env=${env}
        ...    include_in_history=true
        ...    secret_file__kubeconfig=${kubeconfig}
        ...
        IF    ($scaleup.stderr) != ""
            RW.Core.Add Issue
            ...    severity=3
            ...    expected=StatefulSet `${STATEFULSET_NAME}` in namespace `${NAMESPACE}` should scale up successfully
            ...    actual=StatefulSet `${STATEFULSET_NAME}` in namespace `${NAMESPACE}` did not scale up successfully
            ...    title=StatefulSet `${STATEFULSET_NAME}` in `${NAMESPACE}` did not scale up properly
            ...    reproduce_hint=View Commands Used in Report Output
            ...    details=StatefulSet ${STATEFULSET_NAME} in Namespace ${NAMESPACE} generated the following output during scaledown attempt: \n${scaleup.stderr}
            ...    next_steps=Inspect StatefulSet warning Events for `${STATEFULSET_NAME}`\nEscalate rollback issues to service owner.
            ...    observed_at=${timestamp}
...
Increase CPU Resources for StatefulSet `${STATEFULSET_NAME}` in Namespace `${NAMESPACE}`
    ...
    IF    ($sts_check.stderr) != ""
        ...
        ...    reproduce_hint=View Commands Used in Report Output
        ...    details=Error accessing StatefulSet: ${sts_check.stderr}
    ...
    ...    cmd=${KUBERNETES_DISTRIBUTION_BINARY} get vpa -n ${NAMESPACE} --context ${CONTEXT} -o json | jq -r '.items[] | select(.spec.targetRef.name=="${STATEFULSET_NAME}" and (.spec.targetRef.kind=="StatefulSet" or .spec.targetRef.kind=="statefulset")) | .status.recommendation.containerRecommendations[0].upperBound.cpu' 2>/dev/null || echo ""
    ...    env=${env}
    ...    include_in_history=true
    ...    timeout_seconds=180
    ...    secret_file__kubeconfig=${kubeconfig}
    ...
    ...    cmd=${KUBERNETES_DISTRIBUTION_BINARY} get statefulset ${STATEFULSET_NAME} -n ${NAMESPACE} --context ${CONTEXT} -o jsonpath='{.spec.template.spec.containers[0].resources.requests.cpu}'
    ...    env=${env}
    ...    include_in_history=true
    ...    timeout_seconds=180
    ...    secret_file__kubeconfig=${kubeconfig}
    ...
    ...    cmd=${KUBERNETES_DISTRIBUTION_BINARY} get statefulset ${STATEFULSET_NAME} -n ${NAMESPACE} --context ${CONTEXT} -o jsonpath='{.spec.template.spec.containers[0].resources.limits.cpu}'
    ...    env=${env}
    ...    include_in_history=true
    ...    timeout_seconds=180
    ...    secret_file__kubeconfig=${kubeconfig}
    ...
    IF    $is_gitops_managed == "true"
        ...
        RW.Core.Add Issue
        ...    severity=4
        ...    expected=StatefulSet `${STATEFULSET_NAME}` should allow automatic resource updates
        ...    actual=StatefulSet `${STATEFULSET_NAME}` is managed by GitOps and requires manual updates
    ...
    IF    $hpa_exists != ""
        ...
        RW.Core.Add Issue
        ...    severity=4
        ...    expected=StatefulSet `${STATEFULSET_NAME}` should allow resource updates
        ...    actual=StatefulSet `${STATEFULSET_NAME}` has an HPA configured
        ...    title=StatefulSet `${STATEFULSET_NAME}` has HPA - Resource Update Not Recommended
        ...    reproduce_hint=View Commands Used in Report Output
        ...    details=StatefulSet ${STATEFULSET_NAME} in Namespace ${NAMESPACE} has an HPA: ${hpa_exists}. Changing resources may affect HPA behavior.
    ...
    ELSE
        RW.Core.Add Issue
        ...    severity=4
        ...    expected=StatefulSet `${STATEFULSET_NAME}` should have CPU resource requests configured
        ...    actual=StatefulSet `${STATEFULSET_NAME}` has no CPU resource requests set
    ...
    IF    not $suggestion_only and $new_cpu_value != ""
        ...
        END
        ${patch_cmd}=    Set Variable    ${KUBERNETES_DISTRIBUTION_BINARY} set resources statefulset/${STATEFULSET_NAME} -n ${NAMESPACE} --context ${CONTEXT} --requests=cpu=${new_cpu_value}${limits_arg}
        ...
        IF    ($resource_update.stderr) != ""
            RW.Core.Add Issue
            ...    severity=3
            ...    expected=StatefulSet `${STATEFULSET_NAME}` CPU resources should update successfully
            ...    actual=StatefulSet `${STATEFULSET_NAME}` CPU resource update failed
        ...
        ELSE
            ...
            RW.Core.Add Issue
            ...    severity=4
            ...    expected=StatefulSet `${STATEFULSET_NAME}` CPU resources updated
            ...
            ...    next_steps=Monitor workload performance and pod resource utilization\nAdjust resources further if needed based on observed metrics
            ...    observed_at=${timestamp}
        END
    ELSE IF    $new_cpu_value != ""
        ...
        END
        RW.Core.Add Pre To Report    ----------\nSuggested CPU Resource Update (Not Applied):\nRun: ${KUBERNETES_DISTRIBUTION_BINARY} set resources statefulset/${STATEFULSET_NAME} -n ${NAMESPACE} --context ${CONTEXT} --requests=cpu=${new_cpu_value}${limits_suggestion}
    END
...
Increase Memory Resources for StatefulSet `${STATEFULSET_NAME}` in Namespace `${NAMESPACE}`
    ...
    # Check if VPA exists with recommendations
    ${vpa_check}=    RW.CLI.Run Cli
    ...    cmd=${KUBERNETES_DISTRIBUTION_BINARY} get vpa -n ${NAMESPACE} --context ${CONTEXT} -o json | jq -r '.items[] | select(.spec.targetRef.name=="${STATEFULSET_NAME}" and (.spec.targetRef.kind=="StatefulSet" or .spec.targetRef.kind=="statefulset")) | .status.recommendation.containerRecommendations[0].upperBound.memory' 2>/dev/null || echo ""
    ...    env=${env}
    ...    include_in_history=true
    ...    timeout_seconds=180
    ...    secret_file__kubeconfig=${kubeconfig}
    ...
    ...    cmd=${KUBERNETES_DISTRIBUTION_BINARY} get statefulset ${STATEFULSET_NAME} -n ${NAMESPACE} --context ${CONTEXT} -o jsonpath='{.spec.template.spec.containers[0].resources.requests.memory}'
    ...    env=${env}
    ...    include_in_history=true
    ...    timeout_seconds=180
    ...    secret_file__kubeconfig=${kubeconfig}
    ...
    ...    cmd=${KUBERNETES_DISTRIBUTION_BINARY} get statefulset ${STATEFULSET_NAME} -n ${NAMESPACE} --context ${CONTEXT} -o jsonpath='{.spec.template.spec.containers[0].resources.limits.memory}'
    ...    env=${env}
    ...    include_in_history=true
    ...    timeout_seconds=180
    ...    secret_file__kubeconfig=${kubeconfig}
    ...
    ELSE
        RW.Core.Add Issue
        ...    severity=4
        ...    expected=StatefulSet `${STATEFULSET_NAME}` should have memory resource requests configured
        ...    actual=StatefulSet `${STATEFULSET_NAME}` has no memory resource requests set
    ...
    IF    not $suggestion_only and $new_memory_value != ""
        ...
        END
        ${patch_cmd}=    Set Variable    ${KUBERNETES_DISTRIBUTION_BINARY} set resources statefulset/${STATEFULSET_NAME} -n ${NAMESPACE} --context ${CONTEXT} --requests=memory=${new_memory_value}${limits_arg}
        ${resource_update}=    RW.CLI.Run Cli
        ...    cmd=${patch_cmd}
        ...    env=${env}
        ...    include_in_history=true
        ...
        IF    ($resource_update.stderr) != ""
            RW.Core.Add Issue
            ...    severity=3
            ...    expected=StatefulSet `${STATEFULSET_NAME}` memory resources should update successfully
            ...    actual=StatefulSet `${STATEFULSET_NAME}` memory resource update failed
        ...
        ELSE
            ...
            RW.Core.Add Issue
            ...    severity=4
            ...    expected=StatefulSet `${STATEFULSET_NAME}` memory resources updated
            ...
            ...    next_steps=Monitor workload performance and pod memory utilization\nAdjust resources further if needed based on observed metrics\nWatch for OOMKilled events
            ...    observed_at=${timestamp}
        END
    ELSE IF    $new_memory_value != ""
        ...
        END
        RW.Core.Add Pre To Report    ----------\nSuggested Memory Resource Update (Not Applied):\nRun: ${KUBERNETES_DISTRIBUTION_BINARY} set resources statefulset/${STATEFULSET_NAME} -n ${NAMESPACE} --context ${CONTEXT} --requests=memory=${new_memory_value}${limits_suggestion}
    END
...
Decrease CPU Resources for StatefulSet `${STATEFULSET_NAME}` in Namespace `${NAMESPACE}`
    ...
    IF    not $suggestion_only and $new_cpu_value != ""
        ...
        ELSE
            ...
            RW.Core.Add Issue
            ...    severity=4
            ...    expected=StatefulSet `${STATEFULSET_NAME}` CPU resources decreased
...
Decrease Memory Resources for StatefulSet `${STATEFULSET_NAME}` in Namespace `${NAMESPACE}`
    ...
    IF    not $suggestion_only and $new_memory_value != ""
        ...
        ELSE
            ...
            RW.Core.Add Issue
            ...    severity=4
            ...    expected=StatefulSet `${STATEFULSET_NAME}` memory resources decreased
...
Suite Initialization
    ...
    ...    type=string
    ...    description=The multiple in which to increase the total amount of pods. For example, a StatefulSet with 2 replicas and a scale up factor of 2 will result in 4 pods. 
    ...    example=2
    ...    default=2
    ${MAX_REPLICAS}=    RW.Core.Import User Variable    MAX_REPLICAS
    ...    type=string
    ...    description=The max replicas for any scale-up activity on the StatefulSet.  
    ...    example=10
    ...    default=10
    ${ALLOW_SCALE_TO_ZERO}=    RW.Core.Import User Variable    ALLOW_SCALE_TO_ZERO
    ...    type=string
    ...    description=Permit StatefulSets to scale to 0.   
    ...    example=false
    ...    default=false
    ...
    Set Suite Variable
    ...    ${env}
    ...    {"KUBECONFIG":"./${kubeconfig.key}", "KUBERNETES_DISTRIBUTION_BINARY":"${KUBERNETES_DISTRIBUTION_BINARY}", "CONTEXT":"${CONTEXT}", "NAMESPACE":"${NAMESPACE}", "STATEFULSET_NAME": "${STATEFULSET_NAME}", "MAX_REPLICAS": "${MAX_REPLICAS}", "ALLOW_SCALE_TO_ZERO":"${ALLOW_SCALE_TO_ZERO}", "HPA_SCALE_FACTOR":"${HPA_SCALE_FACTOR}", "HPA_MAX_REPLICAS":"${HPA_MAX_REPLICAS}", "HPA_MIN_REPLICAS":"${HPA_MIN_REPLICAS}", "RESOURCE_SCALE_DOWN_FACTOR":"${RESOURCE_SCALE_DOWN_FACTOR}"}
