...
Force Tags          Kubernetes    StatefulSet    Operations
...
Restart StatefulSet `${STATEFULSET_NAME}` in Namespace `${NAMESPACE}`
    [Documentation]    Perform a rollout restart on the StatefulSet
    [Tags]
    ...    log
    ...    pod
    ...    restart
    ...
    IF    ($rollout.stderr) == ""
        ...
        ...    cmd=while true; do STATUS=$(${KUBERNETES_DISTRIBUTION_BINARY} rollout status statefulset/${STATEFULSET_NAME} -n ${NAMESPACE} --context ${CONTEXT} --timeout=180s); echo "$STATUS"; [[ "$STATUS" == *"successfully rolled out"* ]] && break; sleep 5; done
        ...    env=${env}
        ...    include_in_history=false
        ...    timeout_seconds=180
        ...    secret_file__kubeconfig=${kubeconfig}
    ...
    IF    ($rollout.stderr) != ""
        RW.Core.Add Issue
        ...    severity=3
        ...    expected=StatefulSet `${STATEFULSET_NAME}` in namespace `${NAMESPACE}` should have rollout successfully
...
Force Delete Pods for StatefulSet `${STATEFULSET_NAME}` in Namespace `${NAMESPACE}`
    [Documentation]    Force delete all pods related to the StatefulSet using pod template labels
    [Tags]
    ...    log
    ...    pod
    ...    restart
...
Scale Up StatefulSet `${STATEFULSET_NAME}` in Namespace `${NAMESPACE}` by ${SCALE_UP_FACTOR}x
    [Documentation]    Increase StatefulSet replicas by multiplying current count by SCALE_UP_FACTOR (capped by MAX_REPLICAS).
    [Tags]
    ...    scaleup
    ...    statefulset
    ...    access:read-write
    ...
    EXCEPT
        Log    Could not parse replica count from kubectl output; defaulting to 1.    WARN
        ${current_replicas}=    Set Variable    1
    END
...
Increase CPU Resources for StatefulSet `${STATEFULSET_NAME}` in Namespace `${NAMESPACE}`
    ...
    ${statefulset_check}=    RW.CLI.Run Cli
    ...
    IF    ($statefulset_check.stderr) != ""
        ...
        ...    reproduce_hint=View Commands Used in Report Output
        ...    details=Error accessing StatefulSet: ${statefulset_check.stderr}
...
Suite Initialization
    ...
    ...    type=string
    ...    description=Used to target the StatefulSet for queries and filtering events.
    ...    pattern=\w*
    ...    example=artifactory
    ...
    ...    type=string
    ...    description=The multiple by which to increase replica count. For example, a StatefulSet with 2 pods and a scale up factor of 2 will target 4 pods. 
    ...    example=2
    ...    default=2
