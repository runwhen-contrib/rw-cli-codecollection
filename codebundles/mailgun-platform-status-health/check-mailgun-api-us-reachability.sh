...
set -x
# -----------------------------------------------------------------------------
# Unauthenticated probe of the US Mailgun API base. Expects HTTP 401 with a
# JSON body (no API key) — confirms TLS, routing, and that the edge responds.
# Env: MAILGUN_STATUS_REGION_FOCUS — skip when set to "eu" only.
# -----------------------------------------------------------------------------

OUTPUT_FILE="${OUTPUT_FILE:-api_us_reachability_output.json}"
...
if [[ "$MAILGUN_STATUS_REGION_FOCUS" == "eu" ]]; then
  echo '[]' | jq '.' >"$OUTPUT_FILE"
...
if [[ "$curl_rc" -ne 0 ]]; then
  append_issue \
    "Mailgun US API base unreachable (TLS or network failure)" \
    ...
    4 \
    "Check corporate egress, DNS for api.mailgun.net, TLS interception, and Mailgun status before debugging domain configuration."
elif [[ "$http_code" == "000" ]]; then
  append_issue \
    "Mailgun US API base returned no HTTP status" \
    ...
    4 \
    "Investigate network path and proxies; compare with EU reachability task and the public status page."
elif [[ "$http_code" != "401" ]]; then
  append_issue \
    "Mailgun US API base returned unexpected HTTP ${http_code}" \
    ...
    4 \
    "Compare with documented API behavior; check for intercepting proxies, regional routing changes, or ongoing incidents."
else
  if ! echo "$body_head" | jq -e . >/dev/null 2>&1; then
    append_issue \
      "Mailgun US API base response was not JSON" \
      "HTTP 401 body parses as JSON" \
      "Non-JSON body prefix: ${body_head}" \
      "HTTP ${http_code} received but body did not parse as JSON." \
      3 \
      "Validate that traffic reaches Mailgun and is not rewritten by a proxy or HTML error page."
  fi
fi
